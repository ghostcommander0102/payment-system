...
class WalletController extends Controller
{
  ...
  public function postexchangeCoins(Request $request)
    {
        if(isset($request->request_type) && $request->request_type=='buyer'){
            ...
            $data = array(
                ...
                'transaction_id'=>generateUniqueCode()

                );

            DB::table('orders')->insert($data);
        }
        if(isset($request->request_type) && $request->request_type=='seller'){
         $data = array(
                'seller_id'=>Auth::id(),
                'unique_code'=>generateUniqueCode(),
                'buyer_wallet_id'=>'0',
        ...
        $data['coins'] = Coin::where('status', STATUS_ACTIVE)->where('name','MegaCoin')->get();
        $data['title'] = __('Send Coin');
    ...
    public function exchangeCoins(Request $request)
    {
        ...
        return view('user.pocket.send_coin_request')->with($data);
   ...
   // create new wallet
    // public function createOrder(WalletCreateRequest $request)
    // {
